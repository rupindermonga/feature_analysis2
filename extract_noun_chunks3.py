...
def extractNounChunk(path, threshold_num_review, threshold_num_review_per_product, threshold_review_words, min_freq, no_sentences):
        ...
        for doc in docs:
            ...
            for chunk in doc.noun_chunks:
                ...
                if lemma not in ['-PRON-','']:
                    ...
                    # if lemma in sent:
                    # print(1)
                    if lemma not in sentence_dict.keys():
                        sentence_dict[lemma] = [doc]
                        # sentence_out.writerow([lemma, doc])
                    else:
                        if len(sentence_dict[lemma])< no_sentences:
                            sentence_dict[lemma].append(doc)
                            # sentence_out.writerow([lemma,doc])
    ...
    sentence_out.writerow(['noun_chunk', 'sentences'])

    for k, v in sentence_dict.items():
        if overAllFreq[k] > min_freq:
            sentence_out.writerow([k, v])
